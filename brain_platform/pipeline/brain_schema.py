"""Structured brain schema for single-pass personality extraction.
...
# Allowed sensitivity tiers for brain nodes.
SensitivityLevel = Literal["public", "personal", "private"]

# Generic role labels that should be merged into a named person node.
# Shared between brain_extractor and graph_reader for entity resolution.
...
# ── Procedural memory layer (PlugMem) ──────────────────────
# These nodes capture *how* the person works — reasoning, prompting, action
# patterns — independent of any specific tech stack or domain. They form the
# procedural memory channel separate from episodic (memories) and semantic
# (beliefs/expertise) memory.
#
# Source provenance lives on the node itself: "interview" = self-narrated,
# "artifact" = derived from real artifacts (PRs, prompts, etc.). Discrepancies
# between the two sources are real signal (per TinyStyler), not error.
...
class CalibrationKnobs(BaseModel):
    ...
    Defaults give the "improved" projection; raw=passthrough is research mode.
    Applied at agent runtime — not at extraction. See plan/10 §7.
    """
    tone_temperature: float = Field(
        default=1.0,
        description="0.0=neutral delivery, 1.0=raw, >1=amplified. Preserves signal, adjusts edge.",
        ge=0.0, le=2.0,
    )
    confidence_clip: float | None = Field(
        default=None,
        description="If set, cap stated confidence at this value (0-1). Useful for category-2 biases.",
        ge=0.0, le=1.0,
    )
    ...
    factual_correction: bool = Field(
        default=True,
        description="Allow runtime to correct category-1 factual errors. Does not affect biases.",
    )
    harm_gate: Literal["off", "soften", "exclude"] = Field(
        default="soften",
        description="Category-5 harmful-pattern gating. 'off'=expose, 'soften'=tone down, 'exclude'=hide from agent.",
    )


class ProceduralPatternNode(BaseModel):
    """A pattern in *how* the person works. Role-agnostic — captures meta-method,
    not technique. Examples: 'hypothesis-first debugging', 'spec-first prompting',
    'small-PR refactoring'. Aggregated; never stored below the sample threshold."""
    name: str = Field(description="Concise pattern name under 50 chars")
    domain: str = Field(default="", description="Free-text role context (e.g., 'debugging', 'design critique', 'writing')")
    ...
    anti_pattern: str = Field(default="", description="What they explicitly do NOT do in this situation")
...
class WorkLoopNode(BaseModel):
    ...
    Captures how someone iterates through a task, not what tools they use."""
    ...
    stop_condition: str = Field(default="", description="When they stop — finished, frustrated, time-boxed")
...
class PromptingStyleNode(BaseModel):
    """How the person directs an AI or delegates to a person. Captured from
    real prompts (artifact source) or interview narration (interview source).
    Does NOT enumerate tech — captures meta-structure of their requests."""
    ...
    correction_style: str = Field(default="", description="How they course-correct an agent — gentle / direct / restart-from-scratch / clarifying")
    constraint_phrasing: str = Field(default="", description="How they phrase limits and requirements")
    examples_excerpts: list[str] = Field(
        default_factory=list,
        description="0-3 redacted excerpts of their actual prompts (no PII / secrets)",
    )
    source: ProceduralSource = Field(default="interview", description="interview | artifact | synthesis")
    sensitivity: SensitivityLevel = Field(default="public", description="Privacy tier")
...
class TechnicalGapNode(BaseModel):
    """A knowledge gap the person has — what they avoid, are unaware of, or
    hold outdated views on. Aspirational gaps (ones they want to close) are
    flagged; non-aspirational gaps are still preserved as personality data."""
    ...
    evidence: str = Field(default="", description="What revealed the gap — interview quote or artifact pattern")
...
class PersonNode(BaseModel):
    ...
    sensitivity: SensitivityLevel = Field(default="private", description="Privacy tier: public, personal, or private (defaults to private — named people are sensitive)")
...
class ContradictionPattern(BaseModel):
    ...
    topic: str = Field(description="What they'll fight about, e.g. 'biodegradable vs compostable'")
    stance: str = Field(description="Their position, e.g. 'they are NOT the same thing'")
    how_they_push_back: str = Field(default="", description="Their style when correcting, e.g. 'patient but firm, uses specific technical distinctions'")


class WorkDNA(BaseModel):
    """The procedural fingerprint — how this person *works* across tasks.

    Parallel to VoiceDNA but for action and reasoning rather than communication.
    Aggregated across all procedural inputs (interview + artifacts). Role-agnostic:
    the same fields apply whether the person is a coder, designer, writer, etc.
    """
    decomposition_style: str = Field(
        default="",
        description="How they break down problems, e.g. 'top-down with explicit interfaces' vs 'emergent — start with concrete cases'",
    )
    error_taxonomy: str = Field(
        default="",
        description="Which classes of error they assume vs. hunt for, e.g. 'assumes type errors caught upstream, hunts for off-by-one and timezone bugs'",
    )
    debugging_approach: str = Field(
        default="",
        description="How they investigate problems, e.g. 'hypothesis-driven, forms a theory then tests it' vs 'trace-driven, reads logs end-to-end first'",
    )
    review_depth: str = Field(
        default="",
        description="How they review others' work — depth, speed, tone, e.g. 'fast skim for shape, then deep dive on hot paths'",
    )
    documentation_habit: str = Field(
        default="",
        description="What/when/how they document, e.g. 'inline rationale for non-obvious choices, no comments on what code does'",
    )
    abstraction_timing: str = Field(
        default="",
        description="When they introduce abstractions, e.g. 'after the third occurrence, never before' or 'upfront when problem shape is clear'",
    )
    risk_posture: str = Field(
        default="",
        description="Risk preference under uncertainty, e.g. 'ships small reversible changes' vs 'invests in a single durable solution'",
    )
    delegation_style: str = Field(
        default="",
        description="How they hand work off to people or AI, e.g. 'spec + examples + acceptance criteria' vs 'high-level intent and trust'",
    )
    stop_conditions: str = Field(
        default="",
        description="When they decide work is done, e.g. 'when tests pass and the diff explains itself' vs 'time-boxed, ship and iterate'",
    )
...
class VoiceDNA(BaseModel):
    ...
    characteristic_phrases: list[str] = Field(
        default_factory=list,
        description="Actual phrases/expressions they use, e.g. ['endothermic reaction', 'what's the worst realistic outcome']",
    )
    phrases_to_avoid: list[str] = Field(
        default_factory=list,
        description="Things they would NEVER say, e.g. ['let me circle back', 'synergy']",
    )
    punctuation_and_formatting: str = Field(
        default="",
        description="How they text/write, e.g. 'lowercase no punctuation unless angry, then perfect grammar'",
    )
    emoji_usage: str = Field(
        default="",
        description="Emoji habits, e.g. 'never uses emojis' or 'heavy emoji user, especially 💀 and 😭'",
    )
    humor_style: str = Field(
        default="",
        description="How they're funny, e.g. 'self-deprecating, chemistry analogies, dry wit'",
    )
    response_length_pattern: str = Field(
        default="",
        description="How long their messages typically are, e.g. 'either 3 words or 3 paragraphs, nothing in between'",
    )
    formality_range: str = Field(
        default="",
        description="How formality shifts by context, e.g. 'casual with friends, more structured in docs'",
    )
    filler_words: list[str] = Field(
        default_factory=list,
        description="Verbal tics and filler, e.g. ['like', 'you know', 'right?']",
    )
    storytelling_style: str = Field(
        default="",
        description="How they tell stories, e.g. 'builds up slowly then hits you with the punchline, uses vivid sensory details'",
    )
    listener_vs_talker: str = Field(
        default="",
        description="Conversation balance, e.g. 'listens 20min then delivers one long synthesized monologue'",
    )
...
class EdgeSpec(BaseModel):
    ...
    Example: "quitting the agency job" SHAPED "belief in sustainability"
    """
    ...
    def normalize_field_names(cls, v: dict) -> dict:
        """Accept alternate field names that some LLMs produce (e.g. source/target)."""
...
class EmotionalProfile(BaseModel):
    ...
    baseline_mood: str = Field(default="", description="Default emotional state, e.g. 'cautiously optimistic'")
...
class PersonalityGraph(BaseModel):
    ...
    This is the output of a single LLM call over the full interview text.
    ...
    # ── Clone specification (Pass 3) ──
    voice_dna: VoiceDNA | None = Field(default=None, description="Communication fingerprint")
    work_dna: WorkDNA | None = Field(default=None, description="Procedural fingerprint — how they work")
    ...
    # ── Emotional dynamics (Pass 4) ──
    emotional_triggers: list["EmotionalTrigger"] = Field(default_factory=list, description="Situations that reliably produce emotional responses")
...
# ── Edge type → Neo4j label mapping ─────────────────────────
# Maps our direction-clear edge types to Neo4j relationship names.
# Also defines which hub edge type connects THE_USER to each node type.
#
# EDGE_TYPES is now derived from `pipeline.edges.EDGE_REGISTRY` — the
# single source of truth that also drives prompt rendering and validation.
# Re-exported here for backward compatibility with existing imports.

from brain_platform.pipeline.edges import EDGE_TYPES  # noqa: E402,F401
...
HUB_EDGE_FOR_TYPE = {
    ...
    # Procedural memory hub edges — THE_USER WORKS_BY <pattern>
