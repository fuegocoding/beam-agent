...
# Local brain files (e.g. someone shared their exported brain with you)
beam install ~/Downloads/alice.brain.json
beam install /path/to/shared/brain.json
...
The install command accepts a **local file path** in addition to a marketplace slug. The flow for sharing a brain is:

**Exporter (publisher side):**

```bash
$ beam brain platform-export ~/my-brain.json --format jsonld
✓ Brain file exported: ~/my-brain.json
  Size: 334381 bytes
  Hash: a69dc36c7c662f06
  Nodes: 199, Edges: 384
```

**Recipient (consumer side):**

```bash
$ beam install ~/Downloads/my-brain.json
Installing brain from local file: /Users/me/Downloads/my-brain.json
  Format valid: BrainFile v2.2.0 (199 nodes, 384 edges)
✓ Installed: my-brain
  (Auto-ingests into Neo4j if configured)
```

The exported file is a standard **`BrainFileSchema` v2.2.0** JSON document — the same format used by the marketplace. It's portable, self-describing (with `@context` + `@type: BrainFile` JSON-LD markers), and validated by the installer before being written to disk. The format is the same whether the brain was built from the interview, generated by `platform-export`, or downloaded as a marketplace brain.

**Sharing is just "send someone the file"** — email attachment, file share, version control, whatever. No central server, no API key, no special hosting. The recipient runs `beam install <path>` and gets a working brain.
...
| `beam install [slug\|path]` | Install a marketplace brain (no args = interactive picker) **or a local brain file** (`beam install ~/brain.json`). `--list` shows catalog. |
| `beam brain install [slug\|path]` | Same as `beam install`, as a subcommand of `brain` |
