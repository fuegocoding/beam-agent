...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or beam -c.",
    ...
    "Ctrl+Z suspends Beam to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "beam -c resumes your most recent CLI session. beam -c \"project name\" resumes by title.",
    "beam -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "beam -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "beam chat -t web,terminal enables only specific toolsets for a focused session.",
    "beam chat -s github-pr-workflow preloads a skill at launch.",
    "beam chat -q \"query\" runs a single non-interactive query and exits.",
    "beam chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "beam chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "beam --yolo bypasses all dangerous command approval prompts for the entire session.",
    "beam chat --source telegram tags the session for filtering in beam sessions list.",
    "beam -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "beam doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "beam dump outputs a compact setup summary — great for bug reports.",
    "beam config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "beam config edit opens config.yaml in your default editor.",
    "beam config check scans for missing or stale configuration options.",
    "beam sessions browse opens an interactive session picker with search.",
    "beam sessions stats shows session counts by platform and database size.",
    "beam sessions prune --older-than 30 cleans up old sessions.",
    "beam skills search react --source skills-sh searches the skills.sh public directory.",
    "beam skills check scans installed hub skills for upstream updates.",
    "beam skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "beam skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "beam mcp add github --command npx adds MCP servers from the command line.",
    "beam mcp serve runs Beam itself as an MCP server for other agents.",
    "beam auth add lets you add multiple API keys for credential pool rotation.",
    "beam completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "beam logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "beam backup creates a zip backup of your entire Beam home directory.",
    "beam profile create coder creates an isolated profile that becomes its own command.",
    "beam profile create work --clone copies your current config and keys to a new profile.",
    "beam update syncs new bundled skills to ALL profiles automatically.",
    "beam gateway install sets up Beam as a system service (systemd/launchd).",
    "beam memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "beam webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: beam tools disables unused tools, beam skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "beam models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as beam -w).",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Beam tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'beam profile create coder' creates the 'coder' command.",
    "beam profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Beam prints a resume command with session ID and stats.",
    "beam sessions export backup.jsonl exports all sessions for backup or analysis.",
    "beam -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "beam skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .beam/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: beam cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Beam runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "beam gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Beam via a local macOS server.",
    ...
    "Context files (.beam.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "Beam loads project context from .beam.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "beam mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "beam mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "beam mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — beam tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.beam/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "beam chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "beam plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "beam acp runs Beam as an ACP server for VS Code, Zed, and JetBrains integration.",
    ...
    "beam auth add nous or beam auth add openai-codex sets up OAuth-based providers.",
    ...
    "beam status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.beam/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Beam your own.",
    ...
    "Before a gateway session reset, Beam auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.beam/skills/.hub/audit.log tracks every install and removal operation.",
    ...
    "Every interrupt during an agent run is logged to ~/.beam/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'beam profile create ls' would be rejected.",
    "beam profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "beam claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via beam tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.beam/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.beam/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Beam auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Beam config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Beam is working.',
    ...
    '/reload reloads ~/.beam/.env into the running session — pick up new API keys without restarting.',
    ...
    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.beam/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.beam/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'beam curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "beam curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'beam curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'beam auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    'beam dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'beam dashboard embeds the full Beam TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.beam/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.beam/dashboard-plugins/ — no npm build required.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.beam/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.beam/skills/ so drop-in skills appear without restarting the session.',
    ...
    'beam -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'beam chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'beam chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'beam chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "beam chat --source tool tags programmatic chats so they don't clutter beam sessions list.",
    'beam dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'beam sessions rename <ID> "new title" renames any past session; beam sessions delete <ID> removes one.',
    'beam import restores a session export or profile archive produced by sessions export or profile export.',
    'beam fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'beam pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'beam setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'beam status --deep runs the full health sweep across every component; plain beam status is the quick view.',
    ...
    'HERMES_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.beam/plugins/ — trust-gated by design.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Beam block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from beam sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.beam/dashboard-plugins/.',
]
